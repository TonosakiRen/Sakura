...
bool Collider::Collision(Collider& colliderB, Vector3& minAxis, float& minOverlap)
{
	...
	//このゲーム特別処理
	if (std::fabsf(Length(obb1.center - obb2.center)) > 6.0f) {
		return false;
	}
...
bool Collider::Collision(Collider& colliderB, Vector3& pushBuckVector)
{
	...
	//Vector3 priorityVector;
	//bool isDiffHeight = false;
	////相手が高い
	//if (preLeftTop_.y < colliderB.preRightBottom_.y) {
	//	priorityVector = { 0.0f,-1.0f,0.0f };
	//	isDiffHeight = true;
	//}
	////相手が低い
	//if (preRightBottom_.y < colliderB.preLeftTop_.y) {
	//	priorityVector = { 0.0f,1.0f,0.0f };
	//	isDiffHeight = true;
	//}
	//
	...
	//if (isDiffHeight == false) {
	//	if (ObbCollision(obb1, obb2, minAxis, minOverlap)) {
	//		float dot = Dot(MakeTranslation(colliderB.worldTransform_.matWorld_) - MakeTranslation(this->worldTransform_.matWorld_), minAxis);
	//		if (dot > 0.0f) {
	//			minOverlap = -minOverlap;
	//		}
	//		pushBuckVector = Normalize(minAxis) * (minOverlap + 0.0001f);
	//		return true;
	//	}
	//	else {
	//		pushBuckVector = { 0.0f,0.0f,0.0f };
	...
	//}
	//else {
	//	if (ObbCollision(obb1, obb2, minAxis, minOverlap, priorityVector)) {
	//		float dot = Dot(MakeTranslation(colliderB.worldTransform_.matWorld_) - MakeTranslation(this->worldTransform_.matWorld_), minAxis);
	//		if (dot > 0.0f) {
	//			minOverlap = -minOverlap;
	//		}
...
bool Collider::Collision(Collider& colliderB, Vector3& pushBuckVector, const Vector3& priorityAxis)
{
	MatrixUpdate();
	colliderB.MatrixUpdate();
	OBB obb1;
	...
	if (ObbCollision(obb1, obb2, minAxis, minOverlap, priorityAxis)) {
		float dot = Dot(MakeTranslation(colliderB.worldTransform_.matWorld_) - MakeTranslation(this->worldTransform_.matWorld_), minAxis);
		if (dot > 0.0f) {
			minOverlap = -minOverlap;
		}
...
bool Collider::ObbCollision(const OBB& obb1, const OBB& obb2, Vector3& minAxis, float& minOverlap, const Vector3& priorityAxis)
{
	Vector3 normalizePriorityAxis = Normalize(priorityAxis);
	...
	float priorityMinoverlap = FLT_MAX;
	int priorityAxisIndex = 0;
	bool existPriority = false;
	...
	for (int i = 0; i < 15; i++) {
		...
		if (normalizePriorityAxis.x == separationAxisCandidate[i].x &&
			normalizePriorityAxis.y == separationAxisCandidate[i].y && 
			normalizePriorityAxis.z == separationAxisCandidate[i].z) {
			if (overlap < priorityMinoverlap) {
				priorityMinoverlap = overlap;
				priorityAxisIndex = i;
				existPriority = true;
			}
		}
	...
	if (existPriority == true) {
		minAxis = separationAxisCandidate[priorityAxisIndex];
		minOverlap = priorityMinoverlap;
	}
	else {
		minAxis = separationAxisCandidate[axisIndex];
		minOverlap = minoverlap;
...
void Collider::prePosUpdate()
{

	
	Vector3 center = MakeTranslation(worldTransform_.matWorld_);
	Vector3 size = MakeScale(worldTransform_.matWorld_);

	preLeftTop_ = Vector2{ center.x - size.x , center.y + size.y };
	preRightBottom_ = Vector2{ center.x + size.x , center.y - size.y };
}
